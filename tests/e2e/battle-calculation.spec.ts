...
const selectors = {
    ...
    changeOrderCheckbox: "[data-testid='change-order-checkbox']",
...
test.describe("Battle Calculation", () => {
    ...
    // Units with the "onlyExplode" skill (Boomchi/Segment/InsectEgg) always
    // explode and are consumed on attack, so an attacking unit ends at 0 HP
    // when its blast lands — but a unit that never gets to attack (because the
    // defender is already dead) keeps its full health.
    test("onlyExplode attacker is consumed when the defender survives", async ({
        page,
    }) => {
        // Boomchi splashes a full-health warrior for 4 and is consumed itself.
        await setGameVersion(page, "116");
        await goToNextAttackersPage(page);
        await goToNextAttackersPage(page);
        await goToNextAttackersPage(page);
    ...
    test("onlyExplode attackers stop once the defender dies", async ({
        page,
    }) => {
        // The first three Boomchi explode and kill the warrior; the fourth
        // never attacks, so it survives at full health while the rest are 0.
        await setGameVersion(page, "116");
        await goToNextAttackersPage(page);
        await goToNextAttackersPage(page);
        await goToNextAttackersPage(page);
        ...
        await addDefender(page, "warrior");
        await expectDefenderHealth(page, -5);
        await expectAttackerHealth(page, 0, 0);
        await expectAttackerHealth(page, 0, 1);
        await expectAttackerHealth(page, 0, 2);
        await expectAttackerHealth(page, 10, 3);
    });
});
